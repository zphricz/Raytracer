...
enum class GeometryType {NONE, SPHERE, QUAD};
...
class Game {
    private:
        ...
        float detect_triangle_hit(Linear::Vec3f origin, Linear::Vec3f ray, int& triangle_index,
                              float max_distance = std::numeric_limits<float>::max(),
                              float cutoff = ERR_MARGIN);
        float detect_quad_hit(Linear::Vec3f origin, Linear::Vec3f ray, int& quad_index,
                              float max_distance = std::numeric_limits<float>::max(),
                              float cutoff = ERR_MARGIN);
        float detect_sphere_hit(Linear::Vec3f origin, Linear::Vec3f ray, int& sphere_index,
                                float max_distance = std::numeric_limits<float>::max(),
                                float cutoff = ERR_MARGIN);
        float detect_hit(Linear::Vec3f origin, Linear::Vec3f ray,
                                int& index, GeometryType& hit_type);
        float detect_hit(Linear::Vec3f origin, Linear::Vec3f ray);
