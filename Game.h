...
class Game {
  private:
    ...
    float
    detect_quad_hit(Linear::Vec3f origin, Linear::Vec3f ray, int &quad_index,
                    float max_distance = std::numeric_limits<float>::max(),
                    float cutoff = ERR_MARGIN);

    float
    detect_plane_hit(Linear::Vec3f origin, Linear::Vec3f ray, int &plane_index,
                     float max_distance = std::numeric_limits<float>::max(),
                     float cutoff = ERR_MARGIN);

    float
    detect_sphere_hit(Linear::Vec3f origin, Linear::Vec3f ray,
                      int &sphere_index,
                      float max_distance = std::numeric_limits<float>::max(),
                      float cutoff = ERR_MARGIN);
