...
void Game::calculate_quad_normals() {
    for (Quad &quad : quads) {
        ...
        planes.push_back({ quad.p1, quad.normal, quad.reflectivity,
                           quad.spec_power, quad.c });
    }
    quads.clear();
}
        ...
        if (shadow_hit_distance > light_distance) {
            ...
            if (spec_power > 0.0) {
                specular_light_quantity =
                    ...
                    pow(max<float>(reflection.dot(light_vec), 0.0),
                        spec_power) /
                    distance_square;
            }
#if 1
            r += (c.r * diffuse_light_quantity + specular_light_quantity) *
                 weight;
            g += (c.g * diffuse_light_quantity + specular_light_quantity) *
                 weight;
            b += (c.b * diffuse_light_quantity + specular_light_quantity) *
                 weight;
#else
...
#if 1
#if 1
        ray_trace(hit_spot, reflection, r, g, b, weight * reflectivity,
                  depth + 1);
#else
        float rr = 0.0;
        float rg = 0.0;
    ...
    while (running) {
        for (int i = slice * scr->height / nthreads;
            ...
            for (int j = 0; j < scr->width; ++j) {
                ...
                float jitter_x = rand_float(0.5 - jitter_amount / 2.0,
                                            0.5 + jitter_amount / 2.0);
                float jitter_y = rand_float(0.5 - jitter_amount / 2.0,
                                            0.5 + jitter_amount / 2.0);
                Vec3f sample_end =
                    top_left + dx * (jitter_x + j) + dy * (jitter_y + i);
        ...
        if (++i == 20) {
            cout << "RAYS PER SECOND: "
                 << (this_num_rays_cast - num_rays_cast_last) /
                        duration_cast<duration<float> >(current_time -
                                                        last_time)
                            .count()
                 << endl;
